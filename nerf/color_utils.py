...
def read_image_rgb_downsample_yuv422(img_path, H, W, downscale, blend_a=True):
    #read in as RGB, convert to YUV, then convert back into RGB 
    _, y, u, v, H, W = read_image_yuv422_f32(img_path, H, W, downscale, blend_a=True)
...
def read_image_rgb8(img_path, H, W, downscale, blend_a=True):
    '''
    img = imageio.imread(img_path).astype(np.float32)
    # img[..., :3] = srgb_to_linear(img[..., :3])
    if img.shape[2] == 4:  # blend A to RGB
...
def read_image_yuv420_8(img_path, H, W, downscale, blend_a=True):
    '''
    img = imageio.imread(img_path).astype(np.float32) 
    # img[..., :3] = srgb_to_linear(img[..., :3])
    if img.shape[2] == 4:  # blend A to RGB
...
# read and STORE as YUV420 f32 image FROM ORIGINAL PNG IMAGE 
def read_image_yuv420_f32(img_path, H, W, downscale, blend_a=True):
    img = imageio.imread(img_path).astype(np.float32) 
...
# FROM ORIGINAL IMAGE?? 
def read_image_yuv422_8(img_path, H, W, downscale, blend_a=True):
    # from ORIGINAL IMAGE 
    img = imageio.imread(img_path).astype(np.float32) 
    # img[..., :3] = srgb_to_linear(img[..., :3])
    if img.shape[2] == 4:  # blend A to RGB
...
def read_image_yuv422_f32(img_path, H, W, downscale, blend_a=True):
    img = imageio.imread(img_path).astype(np.float32) 
    if H is None or W is None:
        H = image.shape[0] // downscale
        W = image.shape[1] // downscale
